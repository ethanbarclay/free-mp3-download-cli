...
parser.add_argument("-f", action="store_true", help = "Download lossless .flac")
...
def downloadTrack(trackId, albumId):
        ...
        albumDownloadParams = {"i":album["tracks"]["data"][trackIndex]["id"], "ch":key, "f":fileType,}
        headers_dict = {"Host": "free-mp3-download.net", 
        ...
        # get album data
        albumDownload = requests.get("https://free-mp3-download.net/dl.php", albumDownloadParams, headers=headers_dict)
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        with open(folder + "/" + str(trackIndex) + " - " + trackName + "." + fileType, "wb") as f:
            f.write(albumDownload.content)
        print(str(trackIndex) + " - " + trackName + "." + fileType)

        # set track metadata
        localTrack = music_tag.load_file(folder + "/" + str(trackIndex) + " - " + trackName + "." + fileType)
